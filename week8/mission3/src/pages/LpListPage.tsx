...
const LpListPage = () => {
  ...
  const {
    ...
    enabled: debouncedQuery.trim().length > 0 || search.trim().length === 0,
          ...
          <div className="w-full md:max-w-md">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-white/60">
              ...
              {recentSearches.length === 0 ? (
                <span className="text-white/30">최근 검색어가 없습니다.</span>
              ) : (
  ...
  return (
    <section className="min-h-screen bg-black px-8 py-10 text-white">
      ...
      <div className="mb-4 rounded-md border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">
        throttled scrollY: {throttledScrollY}
      </div>
      ...
      {lps.length === 0 && (
        <p className="py-20 text-center text-white/50">검색 결과가 없습니다.</p>
      )}
