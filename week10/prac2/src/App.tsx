...
const CartItem = memo(function CartItem({
  ...
  console.log(`CartItem 렌더링: ${item.title}`);
...
function App() {
  ...
  const total = useMemo(() => {
    console.log("총 금액 계산 실행");

    return cartItems.reduce((totalPrice, item) => {
      return totalPrice + Number(item.price) * item.amount;
    }, 0);
  }, [cartItems]);
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="mx-auto max-w-5xl bg-white px-8 py-6">
        <div className="mb-4 rounded-md bg-pink-50 px-4 py-3 text-sm text-pink-700">
          Practice 2: useMemo를 사용해 장바구니 총 수량과 총 금액 계산 결과를
          메모이제이션합니다.
        </div>
